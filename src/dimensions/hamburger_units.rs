...
dimension!(HAMBURGER_RADIUS =;2.75;INCH);
dimension!(HAMBURGER_HEIGHT =;3.5;INCH);

pub mod base_units {
    use super::*;
    
    dimension!(HAMBURGER_VOLUME =;PI;HAMBURGER_RADIUS^2 HAMBURGER_HEIGHT);
    dimension!(,8,HAMBURGER_MASS = KILOGRAM);
    dimension!(HAMBURGER_ENERGY =;378.75;,Kilo CALORIE);
    dimension!(HAMBURGER_MOLAR_MASS =;18.015;GRAM MOLE^-1);
    dimension!(HAMBURGER_MOLAR_CHARGE =;10;ELEMENTARY_CHARGE AVOGADRO_S_NUMBER); // 10 electrons per molecule of water
    dimension!(HAMBURGER_SPECIFIC_HEAT_CAPACITY = CALORIE GRAM^-1 KELVIN^-1);
    dimension!(HAMBURGER_LUMINOUS_EFFICIENCY =;683.0 * 4.0 * PI;CANDELA WATT^-1);
